...
public:
  WeatherHandler()
    : temperature(0), feelsLike(0), humidity(0),
  ...
  const char* getLine1() {
    if (!dataAvailable) return "NO DATA";
    return line1;
  }
  const char* getLine2() {
    if (!dataAvailable) return "WEATHER";
    return line2;
  }
  bool isAvailable() {
    return dataAvailable;
...
private:
  ...
  switch (displayState) {
    case 0:  // Temperature
      dtostrf(temperature, 4, 1, line1);  // " 24.5"
      strncpy(line2, "C", sizeof(line2)-1);  // Just "C"
      break;
      
    case 1:  // Feels like
      dtostrf(feelsLike, 4, 1, line1);    // " 23.0"
      strncpy(line2, "FL", sizeof(line2)-1); // "FL" for Feels Like
      break;
      
    case 2:  // Humidity
      snprintf(line1, sizeof(line1), "%3d", humidity);  // " 65"
      strncpy(line2, "%", sizeof(line2)-1);            // "%"
      break;
  }
  line2[sizeof(line2)-1] = '\0';  // safety
}
};
